...
def purify_welding_data(file_date):

    # Initialize the variable
    welding_parser_complete = False
    ...
    search_command = "SELECT * FROM %s WHERE Meldung LIKE '%s' OR Meldung LIKE '%s' AND DATE(Datum) = '%s'" % \
                     (table, search_string_std, search_string, file_date)

    # Cycle through the tables containing the data of every month
    try:
        # Execute the SQL command
        cur_main.execute(search_command,)
        ...
        print"*** weld internal completion ***"
